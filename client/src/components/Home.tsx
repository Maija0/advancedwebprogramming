...
return (
    <div>
        <Box sx={{display:"flex", margin:2}}>
            <TextField label="Board name" variant="outlined" value={newBoardName} onChange={(e) => setNewBoardName(e.target.value)}/>
            <Button onClick={createBoard}> Add a board </Button>
        </Box>
    <Box sx={{display:"flex", margin:2}}>
        {boards.map((board) => (            
    <Card sx={{maxWidth:345, minWidth:100, margin: 1, border: '1px solid blue', }} key={board._id}> {/* Fix the ratios or create separate file mobile, pc etc*/}
